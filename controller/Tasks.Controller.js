...
//Post notes
router.post("/create-Task",catchAsyncErrors(async (req, res, next) => {
    try {
        const {
          userId,
tasktitle,
...
taskendtime,
      ...
      const newnote={ 
        ...
        if (result._id) {
          await sendMail({
            email: user.email,
            subject: "New Task Created Successfull",
            message: `Hello ${user.name}, New Task created.\nTask Details below
            \nTask title : ${tasktitle} ,
            \nTask descrition : ${taskdescrp},
            \nTask stratdate : ${stratdate},
          ...
          return res.status(400).json({
            success: false,
          message: "Unable to create new Task, try later.",
        });
        }
...
router.get("/task-notes/:id", async (req, res) => {
  try {
